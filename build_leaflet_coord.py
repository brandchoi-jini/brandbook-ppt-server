...
def _s(v: Any, default: str = "") -> str:
    ...
    if isinstance(v, str):
        t = re.sub(r"[ \t]+", " ", v).strip()
        # ★문자열을 자르고 이어붙이는 과정에서 문장부호 앞에 공백이 남았다.
        #   ("판서 강의로 개념 이식 ." / "1 : 1 개별 첨삭")
        t = re.sub(r"\s+([.,!?;:%)\]}])", r"\1", t)
        t = re.sub(r"([(\[{])\s+", r"\1", t)
...
# ── 학년별 수업 횟수 · 시간 요약 ──────────────────────
# 시간표 표(반×요일·시간)를 리플렛에 그대로 실을 수는 없다.
# 학부모가 상담 전에 알고 싶은 건 <우리 아이 학년은 주 몇 번, 한 번에 몇 시간>이다.
# timetables 의 '요일·시간' 칸에서 학년별로 뽑아 한 줄로 요약한다.
_DAYS = "월화수목금토일"
_TIME_RE = re.compile(r"(\d{1,2})\s*:\s*(\d{2})\s*[~\-–—]+\s*(\d{1,2})\s*:\s*(\d{2})")


def _tt_row_text(r):
    """시간표 한 행 → 문자열. 행은 배열(['반','요일·시간',…])이거나 dict."""
    if isinstance(r, (list, tuple)):
        return " ".join(_s(v) for v in r)
    if isinstance(r, dict):
        return " ".join(_s(r.get(k)) for k in
                        ("time", "day", "days", "when", "schedule", "v", "desc", "name")
                        if r.get(k))
    return _s(r)


# ★요일 글자를 그냥 세면 <수>학·<화>학·<금>요일 아닌 낱글자까지 잡힌다.
#   ('초3 수학A 월수금' → 수학의 '수' 때문에 4회로 셌다)
#   앞뒤가 한글이 아닌 <요일 덩어리>만 인정한다.
_DAY_RUN = re.compile(
    r"(?<![가-힣])([월화수목금토일](?:\s*[,·/~\-]?\s*[월화수목금토일])*)(?![가-힣])")


def _week_count(t):
    """주 몇 회. '주2회' 표기가 있으면 그것을, 없으면 요일 덩어리에서 센다."""
    m = re.search(r"주\s*(\d)\s*회", t)
    if m:
        return int(m.group(1))
    body = _TIME_RE.sub(" ", t)          # 시각의 숫자를 요일로 오인하지 않게 제거
    days = set()
    for run in _DAY_RUN.finditer(body):
        seg = re.sub(r"[\s,·/]", "", run.group(1))
        i = 0
        while i < len(seg):
            ch = seg[i]
            if ch not in _DAYS:
                i += 1
                continue
            # '월~금' 처럼 범위로 적힌 경우 펼친다
            if i + 2 < len(seg) and seg[i + 1] in "~-" and seg[i + 2] in _DAYS:
                a, b = _DAYS.index(ch), _DAYS.index(seg[i + 2])
                if a <= b:
                    days.update(_DAYS[a:b + 1])
                    i += 3
                    continue
            days.add(ch)
            i += 1
...
def _minutes_of(t):
    """회당 분. '17:00~19:00' 또는 '2시간' 표기를 읽는다."""
    m = _TIME_RE.search(t)
    if m:
        h1, m1, h2, m2 = (int(v) for v in m.groups())
        d = (h2 * 60 + m2) - (h1 * 60 + m1)
        return d if 0 < d <= 600 else 0
    m = re.search(r"(\d+)\s*시간\s*(?:(\d+)\s*분)?", t)
    if m:
        return int(m.group(1)) * 60 + int(m.group(2) or 0)
    m = re.search(r"(\d{2,3})\s*분", t)
...
def _fmt_dur(mins):
    ...
    if h and mm:
        return f"{h}시간 {mm}분"
    return f"{h}시간" if h else f"{mm}분"


def _weekly_summary(schema):
    """[(학년 라벨, '주 3회 · 회당 2시간')] — 값이 없는 학년은 건너뛴다."""
    out = []
    for g in (schema.get("timetables") or []):
        if not isinstance(g, dict):
            continue
        label = _s(g.get("group") or g.get("title"))
        cnts, durs = [], []
        ...
        if cnts:
            ...
            parts.append(f"주 {lo}회" if lo == hi else f"주 {lo}~{hi}회")
        if durs:
            lo, hi = min(durs), max(durs)
            parts.append(f"회당 {_fmt_dur(lo)}" if lo == hi
                         else f"회당 {_fmt_dur(lo)}~{_fmt_dur(hi)}")
        if label and parts:
...
# ── 설명 압축 ────────────────────────────────────────
# 지면이 부족할 때 <항목을 빼는> 대신 <설명을 핵심만 남긴다>.
# 앞 문장이 핵심이므로 뒤 문장부터 덜어내고, 문장이 하나뿐이면 절 경계에서 끊는다.
_SENT_SPLIT = re.compile(r"(?<=[.!?])\s+")
_DESC_LIMIT = 70          # 리플렛 답변·설명 목표 길이(자). 문장 단위로만 맞춘다
...
def _sentences(t):
    ...
    if not t:
        return []
    return [x.strip() for x in _SENT_SPLIT.split(t) if x.strip()] or [t]


def _dedupe_sent(t):
    """같은 문장이 두 번 들어온 경우 한 번만 남긴다."""
    out, seen = [], set()
    for p in _sentences(t):
        k = re.sub(r"\s+", "", p)
        if k in seen:
            continue
        seen.add(k)
...
def _core(t, limit):
    """앞 문장부터 limit 자 안에 들어가는 만큼만 남긴다.
    ★문장 중간은 절대 자르지 않는다. 첫 문장 하나가 limit 를 넘으면 그대로 둔다.
      (절 경계에서 끊자 \"주간 테스트를 다시 확인하고 .\" 처럼 문장이 깨졌다)"""
    t = _dedupe_sent(t)
    if len(t) <= limit:
        return t
    out = ""
...
def _shrink_desc(d):
    """설명을 한 단계 더 압축한다. 더 줄일 수 없으면 None.
    ① 중복 문장 제거 → ② 뒤 문장부터 한 문장씩 덜어내기
    문장이 하나뿐이면 더 줄이지 않는다 — 문장을 토막내면 뜻이 깨진다.
    그때는 호출부가 항목 수를 줄인다."""
    d = _s(d)
    if not d:
        return None
    t = _dedupe_sent(d)
    if re.sub(r"\s+", "", t) != re.sub(r"\s+", "", d):
        return t
    sent = _sentences(t)
    if len(sent) > 1:
        return " ".join(sent[:-1])
...
def _faq_list(schema):
    """답이 있는 질문만, 중복 답변은 한 번만 싣는다.
    ★답이 비어 있는 질문("수업 일정이 어떻게 될까요?" 뒤에 아무것도 없음)과
      중등부·고등부처럼 답이 글자까지 같은 질문이 나란히 실려 있었다."""
    qa, qonly, seen = [], [], set()
    for it in _items(schema.get("faq")):
        ...
        if not a:
            qonly.append({"q": q, "a": ""})
            continue
        a = _core(a, _DESC_LIMIT)
        key = re.sub(r"\s+", "", a)
        if key in seen:
            continue
        seen.add(key)
        qa.append({"q": q, "a": a})
    return (qa or qonly)[:6]
...
def _aux_taken(schema):
    ...
    ★통합 패널(학원 안내)은 <계획 단계>에서, 보조 카드는 <그리는 단계>에서
      섹션을 고르기 때문에 서로를 몰랐다. 그 결과 자주 묻는 질문이
      왼쪽 보조 카드(답변까지)와 가운데 통합 패널(질문만)에 두 번 실리고,
      통합 패널은 세 블록이 몰려 설명이 전부 접혔다.
      통합 패널이 쓸 섹션을 여기서 함께 막는다."""
    if not isinstance(schema, dict):
        return set()
    taken = set(schema.get("_auxUsed") or set())
    taken |= set((schema.get("_auxAssign") or {}).values())
    taken |= set(schema.get("_comboKeys") or set())
    return taken


def _aux_peek(schema, host, cands):
    """이 면에 보조 카드가 붙을지 <미리> 알아본다(사용 처리는 하지 않는다).
    ★자리를 잡기 전에 알아야 한다. 예전에는 본문이 면 끝까지 쓰고 나서
      보조 카드를 넣으려 해, 학원 규정이 0.3in 에 밀려 라벨만 남거나
      0.95in 미달로 아예 사라졌다."""
    if not isinstance(schema, dict):
        return None
...
def _aux_release(schema, key):
    """자리가 없어 못 그린 보조 카드는 사용 표시를 되돌린다.
    ★배정만 하고 그리지 못해 <학원 규정이 통째로 사라지던> 문제를 막는다."""
    if not isinstance(schema, dict) or not key:
        return
    used = set(schema.get("_auxUsed") or set())
    used.discard(key)
    schema["_auxUsed"] = used
    aux = dict(schema.get("_auxAssign") or {})
    for h, g in list(aux.items()):
        if g == key:
            aux.pop(h, None)
...
def _draw_aux_card(slide, schema, x, c, key, y_top, y_bot):
    ...
    if not rows or (y_bot - y_top) < 0.72:
        return 0.0
...
def _panel_admission(slide, schema, x, c):
    ...
    # ★보조 카드(학원 규정 등)가 붙는다면 그 자리를 먼저 떼어 둔다.
    #   헤더 바 0.32 + 3항목 설명 = 약 2.0in 이 필요하다.
    _aux_pending = _aux_peek(schema, "admission", ("rules", "specials", "faq"))
    if _aux_pending and not _has_contact:
        Y_END = min(Y_END, 5.34)
    n = max(1, len(steps))
...
def _panel_features(slide, schema, x, c, name):
    ...
    grow = 1.0
    # ★_auxAssign 에 배정된 경우만 보고 있어, 폴백으로 붙는 특강 카드는
    #   자리를 못 받아 첫 항목만 설명이 나왔다.
    _aux_pending_f = _aux_peek(schema, "features", ("faq", "rules", "specials", "schedule"))
    _has_aux = bool(_aux_pending_f)
    if _has_aux:
        Y_END = min(Y_END, 5.60)
        avail = Y_END - Y0
...
def _panel_curriculum(slide, schema, x, c):
    ...
    _derived = False
    if not sched:
        # ★schedule 필드는 앱이 채우지 않아 이 블록이 한 번도 나오지 않았다.
        #   시간표에서 학년별 <주 몇 회 · 회당 몇 시간>을 뽑아 대신 싣는다.
        sched = [{"k": k, "v": v} for k, v in _weekly_summary(schema)]
        _derived = bool(sched)
    if sched:
        # ★커리큘럼 끝과 간격이 벌어져 시간표 행이 잘렸다 → 바짝 붙인다.
        sy0 = max(y + 0.02, 5.10)
        _rect(slide, x + LEFT, sy0, CW, 0.012, fill=c["card_line"])
        _label(slide, x, "학년별 수업 횟수 · 시간" if _derived else "수업 요일 · 시간",
               c, y=sy0 + 0.10)
        sy = sy0 + 0.48
    ...
    # ── 남는 자리를 보조 카드로 채운다 ──
    #   이 면은 단계가 3개면 하단 2.5in 가 남는다. 학원 규정처럼 짧은 섹션을
    #   여기 두면 통합 면이 2블록으로 줄어 FAQ 가 답변까지 들어간다.
    #   ※카드 머리바 0.80in 를 빼고도 1.0in 이 남을 때만 넣는다(잘림 방지).
    _cy = max(y, locals().get("sy", y))
    _aux = _aux_choose(schema, "curriculum", ("rules", "specials"))
    if _aux:
        if _cy + 1.90 < 7.56:
            _draw_aux_card(slide, schema, x, c, _aux, _cy + 0.30, 7.56)
        else:
            _aux_release(schema, _aux)
...
def _panel_management(slide, schema, x, c):
    ...
    # ── 남는 자리를 보조 카드로 채운다 ──
    #   ★보조 카드를 받는 면이 강점·상담 둘뿐이었다. 배치 계획이 이 면에
    #     학원 규정을 배정해도 그리는 코드가 없어 <규정이 통째로 사라졌다>.
    _aux = _aux_choose(schema, "management", ("rules", "specials"))
    if _aux:
        if y + 1.90 < 7.56:
            _draw_aux_card(slide, schema, x, c, _aux, y + 0.34, 7.56)
        else:
...
# ── 예비 패널(대체 섹션) ──────────────────────────────
def _fill_leftover(slide, schema, x, c, y0, y_end, always_map=False):
    ...
    if not img:
        ...
        if isinstance(_ph, list) and _ph:
            ...
            cap = "학원 전경"
    if not img and not always_map:
        return y0
    if not img:
        # ★약도·사진이 아직 없어도 <자리>는 만들어 둔다.
        #   뒷면 하단이 통째로 비어 인쇄 시안이 성의 없어 보였다.
        cap = "오시는 길 (약도 자리)"
...
def _panel_achievements(slide, schema, x, c):
    ...
    # ★뒷면은 손에 들었을 때 가장 먼저 보이는 면이다. 실적이 연한 카드 안
    #   10pt 한 줄로 들어가 눈에 걸리지 않았다.
    #   <도달한 결과>를 크게 앞세우고 무엇이 달라졌는지를 아래에 작게 둔다.
    GH, GAPI, GAPG = 0.28, 0.20, 0.30      # 소제목·항목·그룹 간격
    LN_H = 0.014                           # 항목 사이 헤어라인

    def _split_change(chg):
        """'3등급 → 1등급' → ('1등급', '3등급 에서')  — 도달값을 앞세운다."""
        t = _s(chg)
        if not t:
            return "", ""
    ...
    def _measure(big, small):
        out, tot = [], 0.0
        for gname, items in groups:
            hs = []
            for ttl, chg in items:
                goal, base = _split_change(chg)
                head = goal or ttl
                sub_parts = [p for p in ((ttl if goal else ""), base) if p]
                sub = "  ·  ".join(sub_parts)
                hh = _need_h(head, CW, big, pad=0.02)
                sh = _need_h(sub, CW, small, pad=0.02) if sub else 0.0
                hs.append((head, sub, hh, sh))
            blk = sum(h + (sb + 0.02 if sb else 0.0) for _a, _b, h, sb in hs) \
                  + (GAPI + LN_H) * max(0, len(hs) - 1)
            out.append((gname, hs, blk))
            tot += (GH if gname else 0.0) + blk + GAPG
        return out, (tot - GAPG if out else 0.0)

    _big, _small = 15.0, 9.5
    meas, tot = _measure(_big, _small)
    while tot > AV and _big > 11.0:
        _big -= 0.5
        _small = max(8.5, _small - 0.2)
        meas, tot = _measure(_big, _small)

    _left = []
    for gname, hs, blk in meas:
        if y + (GH if gname else 0.0) + blk > LIMIT + 0.12:
            _left.extend(_a for _a, _b, _h, _s2 in hs)   # 못 실은 실적은 뒤에서 쓴다
            continue
        if gname:
            _text(slide, gname, x + LEFT, y, CW, 0.26,
                  size=9.5, color=c["label"], bold=True, fit=False)
            y += GH
        for i, (head, sub, hh, sh) in enumerate(hs):
            if i:
                _rect(slide, x + LEFT, y, CW, LN_H, fill=c["card_line"])
                y += LN_H + GAPI * 0.5
            _text(slide, head, x + LEFT, y, CW, hh,
                  size=_big, color=c["badge"], bold=True, min_size=11.0,
                  line_spacing=1.14)
            y += hh
            if sub:
                _text(slide, sub, x + LEFT, y + 0.02, CW, sh,
                      size=_small, color=c["body"], min_size=8.0, line_spacing=1.24)
                y += sh + 0.02
            if i < len(hs) - 1:
                y += GAPI * 0.5
        y += GAPG
    ...
    if LIMIT - y >= 1.10:
        y = _fill_leftover(slide, schema, x, c, y, LIMIT, always_map=True)
...
# ★리플렛은 홍보물이다. <수업 · 관리 · 실적>이 전용 면을 갖고 크게 보여야 한다.
#   예전 순서는 상담·등록과 FAQ 가 앞에 있어서 <학습 관리>가 통합 면으로 밀렸고,
#   4항목이 라벨만 남거나 2항목으로 줄었다.
#   상담·등록은 연락처·QR 이 실적 면(뒷면)과 표지·푸터에 이미 들어가므로 통합 면으로 옮긴다.
SECTION_POOL = [
    ("features",     _has_features,     None),                  # 바깥 좌 — name 인자 필요
    ("achievements", _has_achievements, _panel_achievements),   # 바깥 중 = 뒷면 (실적+연락처+QR)
    ("curriculum",   _has_curriculum,   _panel_curriculum),     # 안쪽 좌
    ("management",   _has_management,   _panel_management),     # 안쪽 중
...
def _plan_panels(schema, cand, slots):
    ...
    aux = {}
    #   ★짝을 새 순서에 맞춘다. 특강은 강점 면 하단, 규정은 커리큘럼·관리 면 하단.
    #     통합 면에는 상담·등록과 FAQ 만 남겨 FAQ 가 답변까지 들어갈 높이를 갖는다.
    #   ※보조 카드를 그릴 수 있는 면만 host 로 쓴다(강점·학습 관리·상담).
    #     그리지 못하는 면에 배정하면 그 섹션이 통째로 사라진다.
    for host, guest in (("features", "specials"), ("admission", "rules"),
                        ("features", "rules")):
...
def _panel_combo(slide, schema, x, c, parts):
    ...
    # ① 글자 크기를 줄여 맞춘다
    #   ★설명을 접어서 맞추는 방식은 폐기했다. 질문에 답이 없고 라벨에 설명이
    #     없는 면이 반복해서 나왔다 — 이것이 "자주 묻는 질문·학습 관리가 잘린다"의
    #     실체였다.
    #   맞추는 순서: 글자 크기 → 설명을 핵심만 남기기 → 항목 덜어내기.
    #   질문을 빼기 전에 답을 짧게 줄인다. 질문 수를 지키는 쪽이 상담에 쓸모 있다.
    work = [(t, list(r)) for t, r in blocks]
    nodesc = set()

    def _fit(bl):
        """블록 목록을 글자 크기로 맞춘다 → (size, meas, total)"""
        _sz = BODY
        while True:
            _m, _t = _plan(bl, _sz, nodesc)
            if _t <= AVAIL or _sz <= MIN_BODY:
                return _sz, _m, _t
            _sz -= 0.5

    size, meas, total = _fit(work)

    def _shrink_one():
        """설명이 가장 긴 항목을 한 단계 압축한다. 더 줄일 게 없으면 False."""
        tgt, best = None, 0
        for bi, (_t, rows) in enumerate(work):
            _w = 1.6 if _t in _LOW_PRIO else 1.0   # 부수 블록을 먼저 줄인다
            for ri, (_rt, rd) in enumerate(rows):
                if rd and len(rd) * _w > best:
                    best, tgt = len(rd) * _w, (bi, ri)
        if tgt is None:
            return False
        bi, ri = tgt
        nd = _shrink_desc(work[bi][1][ri][1])
        if nd is None:
            return False
        work[bi][1][ri] = (work[bi][1][ri][0], nd)
        return True

    dropped = []          # 되살리기용 — [(블록 인덱스, 항목)]

    # ★리플렛은 홍보물이다. 자리가 부족하면 <규정> 같은 부수 정보를 먼저 줄이고
    #   자주 묻는 질문·상담 안내는 마지막에 건드린다.
    _LOW_PRIO = ("학원 규정", "특강 및 기타 수업")

    def _drop_one(min_rows):
        """항목 하나를 뺀다. 부수 블록 → 항목이 많은 블록 순으로 고른다."""
        cand = [i for i, (t, r) in enumerate(work)
                if len(r) > min_rows.get(t, 1)]
        if not cand:
            return False
        low = [i for i in cand if work[i][0] in _LOW_PRIO]
        pool = low or cand
        i = max(pool, key=lambda j: (len(work[j][1]), -j))
        dropped.append((i, work[i][1].pop()))
        return True

    # ② 넘치면 <설명을 핵심만 남긴다>. 항목을 임의로 빼는 것보다 낫다.
    #    중복 문장 제거 → 뒤 문장부터 한 문장씩 덜어내기.
    #    문장 중간은 자르지 않는다(뜻이 깨진다).
    while total > AVAIL and _shrink_one():
        size, meas, total = _fit(work)

    # ③ 압축을 다 했는데도 넘치면 그때 항목을 덜어낸다. 블록별 최소 개수는 지킨다.
    _MIN_ROWS = {"자주 묻는 질문": 3, "학습 관리": 2, "상담 · 등록 안내": 2}
    while total > AVAIL and _drop_one(_MIN_ROWS):
        size, meas, total = _fit(work)

    # ④ 그래도 넘치면 블록당 1항목까지 더 덜어낸다.
    while total > AVAIL and _drop_one({}):
        size, meas, total = _fit(work)

    # ⑤ 최후 — 블록당 1항목인데도 넘치면 그때만 설명을 접는다(뒤 항목부터).
    while total > AVAIL:
        alive = [(bi, ri) for bi, (_t, r) in enumerate(work)
                 for ri in range(len(r))
                 if r[ri][1] and (bi, ri) not in nodesc]
        if not alive:
            break
        nodesc.add(alive[-1])
        size, meas, total = _fit(work)

    # ⑥ 높이 실측이 보수적이라 자리가 남는데도 항목을 잃는 경우가 있다.
    #    (실측 예: 3+3 으로 줄인 뒤 0.9in 이 남았다)
    #    아래가 비면 마지막에 뺀 항목부터 되살린다.
    while dropped:
        bi, row = dropped[-1]
        work[bi][1].append(row)
        _sz, _m, _t = _fit(work)
        if _t <= AVAIL:
            dropped.pop()
            size, meas, total = _sz, _m, _t
        else:
            work[bi][1].pop()
            break
...
def build(schema: Dict[str, Any], palette: str = "sewon_teal",
    ...
    if len(cand) >= 5:
        ...
        # ★남는 섹션을 전부 한 면에 몰면 블록이 3개가 되어, 설명이 들어갈
        #   높이가 없어 제목만 남았다(FAQ 답변·학습관리 설명이 통째로 사라짐).
        #   짧은 섹션 하나는 첫 면 하단 보조 카드로 옮겨 통합 패널을 2블록으로 만든다.
        _aux = {}
        if len(rest) >= 3:
            _host = cand[0][0]
            for _guest in ("rules", "specials"):
                if _guest in rest:
                    _aux[_host] = _guest
                    rest.remove(_guest)
                    break
        if isinstance(schema, dict):
            schema["_auxAssign"] = _aux
            schema["_comboKeys"] = set(rest)
