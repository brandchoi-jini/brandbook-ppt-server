...
def _items(value: Any) -> List[Dict[str, Any]]:
    """dict({items|stages}) 또는 list 를 dict 리스트로 정규화."""
    if isinstance(value, dict):
        value = value.get("items") or value.get("stages") or []
...
def _footer(slide, x, name, phone, c):
    ...
    _text(slide, line, x + 0.17, 7.92, PANEL - 0.2, 0.23,
...
def _curriculum_list(schema):
    ...
    for it in stages[:3]:
        ...
        desc = _s(it.get("desc") or it.get("description") or it.get("subj"))
        if not (name or desc):
            continue
        out.append({"name": _clip(name.replace("부", ""), 8),
...
def _panel_admission(slide, schema, x, c):
    ...
    _text(slide, _first(admission.get("subhead") if isinstance(admission, dict) else "",
          ...
          x + 0.29, 1.38, PANEL - 0.58, 0.31, size=9.5, color=c["body"])
    ...
    for i, it in enumerate(steps):
        _chip(slide, x + 0.02, y + 0.06, 0.35, f"{i+1:02d}", c)
        _text(slide, it["title"], x + 0.5, y, PANEL - 0.7, 0.29,
              size=11, color=c["title"], bold=True)
        _text(slide, it["desc"], x + 0.5, y + 0.31, PANEL - 0.7, 0.44,
              size=8.5, color=c["body"])
        y += 0.96

    # 구분선
    _rect(slide, x + 0.29, 5.06, PANEL - 0.58, 0.012, fill=c["card_line"])
    phone = _s(ac.get("phone"))
    _text(slide, f"전화  {phone}" if phone else "전화 상담", x + 0.29, 5.29,
          PANEL - 0.58, 0.29, size=11, color=c["head"], bold=True)
    loc = _first(ac.get("location"), ac.get("address_short"), default="상담 시 위치를 안내해 드립니다.")
    _text(slide, _wrap(loc, 23, 3), x + 0.29, 5.65, PANEL - 0.58, 0.9,
          size=9, color=c["body"])
...
def _panel_faq(slide, schema, x, c, name):
    ...
    for i, it in enumerate(faqs):
        _text(slide, f"Q{i+1}. {it['q']}", x + 0.29, y, PANEL - 0.58, 0.35,
              size=10, color=c["head"], bold=True)
        _rect(slide, x + 0.29, y + 0.4, PANEL - 0.58, 0.75, fill=c["soft"], radius=True)
        _text(slide, it["a"], x + 0.44, y + 0.5, PANEL - 0.86, 0.56,
              size=8.5, color=c["body"])
        y += 1.37


def _panel_cover(slide, schema, x, c, assets):
    ac = schema.get("academy") or {}
...
def _panel_features(slide, schema, x, c, name):
    ...
    for i, it in enumerate(feats):
        _chip(slide, x, y, 0.35, f"{i+1:02d}", c)
        _text(slide, it["title"], x + 0.52, y, PANEL - 0.82, 0.35,
              size=12, color=c["title"], bold=True)
        _text(slide, it["desc"], x + 0.52, y + 0.38, PANEL - 0.82, 0.4,
              size=8.5, color=c["body"])
        y += 0.92
...
def _panel_curriculum(slide, schema, x, c):
    ...
    y = 1.73
    for it in cur:
        _rect(slide, x + 0.29, y, PANEL - 0.58, 1.38, fill="FFFFFF",
              line=c["card_line"], radius=True)
        _rect(slide, x + 0.48, y + 0.19, 0.79, 0.29, fill=c["accent2"], radius=True)
        _text(slide, it["name"], x + 0.48, y + 0.19, 0.79, 0.29,
              size=9.5, color="FFFFFF", bold=True, align=PP_ALIGN.CENTER,
              valign=MSO_ANCHOR.MIDDLE)
        _text(slide, it["desc"], x + 0.48, y + 0.56, PANEL - 0.96, 0.6,
    ...
    if stext:
        _text(slide, _first(_s((schema.get("specials") or {}).get("head")), default="특별 프로그램"),
              x + 0.29, 7.1, PANEL - 0.58, 0.25, size=9, color=c["label"], bold=True)
        _text(slide, _clip(stext, 60), x + 0.29, 7.38, PANEL - 0.58, 0.42,
              size=8.5, color=c["body"])
...
def _panel_management(slide, schema, x, c):
    ...
    if steps:
        _text(slide, _first(_s((schema.get("management") or {}).get("subhead")),
              ...
              x + 0.29, 1.48, PANEL - 0.58, 0.27, size=9.5, color=c["body"], bold=True)
    y = 1.95
    for i, txt in enumerate(steps):
        _chip(slide, x + 0.02, y + 0.03, 0.35, str(i + 1), c)
        _rect(slide, x + 0.52, y, PANEL - 0.82, 0.75, fill=c["soft"], radius=True)
        _text(slide, txt, x + 0.67, y + 0.08, PANEL - 1.12, 0.56,
    ...
    if lines:
        _text(slide, head, x + 0.29, 6.1, PANEL - 0.58, 0.29,
              size=10.5, color=c["head"], bold=True)
        _text(slide, "\n".join(lines), x + 0.29, 6.5, PANEL - 0.58, 1.3,
              size=8.5, color=c["body"])
...
def build(schema: Dict[str, Any], palette: str = "sewon_teal",
    ...
    _panel_admission(s1, schema, 0.0, c)
    _footer(s1, 0.0, name, phone, c)
    # 패널 구분(연한 세로선)
    _rect(s1, PANEL, 0, 0.008, H_IN, fill=c["card_line"])
    _panel_faq(s1, schema, PANEL, c, name)
    _footer(s1, PANEL, name, phone, c)
    _panel_cover(s1, schema, PANEL * 2, c, _assets)

    # ── 슬라이드 2: 안쪽면 (강점 / 커리큘럼 / 관리) ──
    s2 = prs.slides.add_slide(blank)
    _panel_features(s2, schema, 0.0, c, name)
    _footer(s2, 0.0, name, phone, c)
    _rect(s2, PANEL, 0, 0.008, H_IN, fill=c["card_line"])
    _panel_curriculum(s2, schema, PANEL, c)
    _footer(s2, PANEL, name, phone, c)
    _rect(s2, PANEL * 2, 0, 0.008, H_IN, fill=c["card_line"])
    _panel_management(s2, schema, PANEL * 2, c)
    _footer(s2, PANEL * 2, name, phone, c)
