...
def slide_targets(prs, pal, d):
    ...
    for i, it in enumerate(items):
        ...
        # 우 사진 — 행 중앙 정렬 (실제 이미지 있으면 삽입)
        pw = 1.85; ph_h = rh-0.4
        rect(s, 11.9+PADX-pw, cy-ph_h/2, pw, ph_h, PH_BG, radius=True)
        _photos = (d.get("assets",{}) or {}).get("photos", []) or []
        _ph = _photos[i] if i < len(_photos) else (_photos[0] if _photos else "")
        if _ph and place_image(s, _ph, 11.9+PADX-pw, cy-ph_h/2, pw, ph_h, cover=True):
            pass
        else:
...
def slide_admission(prs, pal, d):
    ...
    for i, st in enumerate(steps):
        ...
        # 큰 숫자 (블록 세로 중앙)
        txt(s, PADX, y+block_h/2-0.55, 1.4, 1.1, [(st["no"], num_size, True, pal["accent"])],
            align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        # 제목
        txt(s, PADX+1.7, y+0.16, 9.8, 0.5, [(st["title"], title_size, True, INK_STRONG)])
        # 항목 (hanging indent — "– " 뒤에 둘째 줄 맞춤)
        items = st.get("items", [])
        paras(s, PADX+1.7, y+0.66, 9.9, block_h-0.6,
