...
def slide_management(prs, pal, d):
    ...
    # 행 세로 분배: 항목 수에 맞춰 상하 밸런스 (많으면 촘촘, 적으면 넉넉)
    ...
    for idx, col in enumerate(cols):
        ...
        for j, row in enumerate(col["rows"]):
            ...
            # 설명 (왼쪽 줄맞춤 통일)
            vv = row["v"]
            # 한 과목이면 폭이 넓으니 한 줄당 글자수 계수를 폭에 맞게
...
def slide_rules(prs, pal, d):
    ...
    kw = 0.9
    # 우측 항목 묶음 높이 → 세로 중앙 정렬
    area_top = 2.05; area_bottom = 7.05
    avail = area_bottom - area_top
    gap = min(1.15, avail/max(n,1))
    block = gap*n
    top = area_top + (avail - block)/2
    mid = top + block/2       # 항목 묶음의 세로 중앙

    # 좌 제목 — 항목 묶음과 같은 세로 중앙에 맞춤
    label(s, pal, "관리 지침", x=PADX, y=mid-1.05)
    txt(s, PADX, mid-0.72, 3.35, 1.6,
    ...
    for it in items:
        ...
        txt(s, rx+kw+0.28, cyi-0.45, rw-kw-0.28, 0.9, [(vv, vsize, False, INK)],
