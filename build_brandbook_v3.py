...
def slide_targets(prs, pal, d):
    ...
    for i, it in enumerate(items):
        ...
        block_h = 0.72
        by = cy - block_h/2 - 0.05
        txt(s, cx, by, cw, 0.6, [(subj, ssize, True, c)], line_spacing=1.2)
        txt(s, cx, by+_subj_h(subj,cw)+0.02, cw, 0.5, [(it.get("desc",""), 13.5, False, INK)], line_spacing=1.3)
        # 우 사진 — 행 중앙 정렬
        pw = 2.2; ph_h = rh-0.4
...
def slide_closing(prs, pal, d):
    ...
    # 연락처(각 한 줄)
    paras(s, PADX, 2.75, 6.0, 1.6, [
        [("전화 문의    ", 15, True, pal["navy"]), (a.get("phone",""), 15, False, INK)],
        [("위치    ", 15, True, pal["navy"]), (a.get("address_short",""), 15, False, INK)],
        [("운영시간    ", 15, True, pal["navy"]), (a.get("hours",""), 15, False, INK)],
    ], line_spacing=1.3, para_space=8)
    # QR
