...
try:
    from to_schema import convert as _convert
except Exception:
    _convert = None
...
async def build(req: Request):
    ...
    # 유미니 원본 JSON이면(basic 키 존재) 표준 스키마로 자동 변환.
    # 앱이 STEP2~5에서 만든 브랜드 요소(slogan/catch/identity 등)는 content['brand']로 전달.
    if _convert and content.get('basic') is not None and content.get('academy') is None:
        brand = content.get('brand') or {}
        converted = _convert(content, brand)
        converted['kind'] = kind
        if design: converted['design'] = design
        content = converted
    # 임시 출력 파일
