...
def convert(raw, brand=None):
    ...
    days_used = [d for d in DAY_ORDER if any(d in v for v in slot_map.values())]
    timetable = {}
    if days_used and slot_map:
        rows = []
        for st in sorted(slot_map.keys())[:8]:     # 최대 8개 시간대
            row = [st] + [slot_map[st].get(d, '') for d in days_used]
            rows.append(row)
        # 표 첫 열은 시간대 → 헤더도 맨 앞에 빈칸(또는 '시간')을 둔다
        timetable = {'headers': ['시간'] + days_used, 'rows': rows}
