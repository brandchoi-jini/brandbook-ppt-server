...
TITLE_MAP = {
    ...
    '학원의 주요 실적과 성장 사례를\n보여주세요': '{name}의 주요 실적',
    '학원의 레벨별 반구성과 학습 체계': '{name}의 반 구성과 학습 체계',
    '학원의 반구성과 학습 체계': '{name}의 반 구성과 학습 체계',
    '반구성과 학습 체계': '{name}의 반 구성과 학습 체계',
...
def build(content, out_path, template_dir='.'):
    ...
    for _s in list(prs.slides):
        for _sh in _s.shapes:
            if not _sh.has_text_frame: continue
            txt = _sh.text_frame.text or ''
            if len(txt) < 30: continue
            try:
                w_in = _in(_sh.width); h_in = _in(_sh.height)
            except Exception:
                w_in, h_in = 5, 1.5
            # 대략적 수용 글자수: 폭*높이 기반 (한글 기준)
            cap_chars = max(20, int(w_in * h_in * 8))
            _autosize(_sh, max_chars_at_base=cap_chars, min_pt=9)
