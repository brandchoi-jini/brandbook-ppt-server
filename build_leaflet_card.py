...
def _ach_row(slide, x, y, label, val, w=CWID, pw=1.14):
    """실적 한 줄 — 도달 숫자를 크게, 출발점을 작게.
    ★남는 자리를 채우려고 억지로 늘리지 않고, 있는 값의 강약만 준다."""
    label = _s(label)
    src, dst = _split_change(val)
    vw = w - pw - 0.16
    dh = max(0.28, _need_h(dst, vw, 13.0, line_spacing=1.20, pad=0.02))
    sh = 0.22 if src else 0.0
    _hair(slide, x, y, w)
    y += 0.07
    if label:
        _pill(slide, x, y + 0.03, label, w=pw)
    if dst:
        _text(slide, dst, x + pw + 0.12, y, vw, dh, size=13.0, color=INK,
              bold=True, line_spacing=1.20, min_size=10.0)
    if src:
        _text(slide, src + "에서", x + pw + 0.12, y + dh, vw, sh,
              size=TINY_PT, color=MUTED, fit=False)
    return y + dh + sh + 0.12
