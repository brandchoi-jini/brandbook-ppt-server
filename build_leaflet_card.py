...
def _step_card(slide, x, y, no, title, desc, w=CWID, h=None):
    ...
    # ★높이를 고정으로 받으면 설명 상자가 카드보다 커져 글자가 카드 밖으로
    #   삐져나갔다. 카드 안쪽으로 묶고, 넘치는 분량은 _text 가 줄여 담는다.
    if h:
        dh = max(0.0, min(dh, ch - 0.44))
        # ★_text 는 하한 글자크기에 걸리면 상자를 늘려서라도 다 담는다.
        #   그래서 높이만 묶어도 카드 밖으로 삐져나갔다 → 분량을 미리 자른다.
        if desc and dh > 0:
            _lh = TINY_PT * 1.26 / 72.0                 # 한 줄 높이
            _lines = max(1, int(dh / _lh))
            _cpl = max(8, int(tw / (TINY_PT / 72.0)))   # 한 줄 글자 수
            desc = _clip_sent(desc, _lines * _cpl)
...
def _soft_row_h(label, desc, right="", w=CWID):
    ...
    return max(0.62, 0.36 + dh + 0.12) + 0.10


def _soft_row(slide, x, y, label, desc, right="", w=CWID, pw=None):
    """연회색 행: 캡슐 라벨 + 설명(+오른쪽 과목). 다음 y 를 반환.
    ★라벨을 캡슐로 바꿔 앞면·뒷면이 같은 어휘를 쓰게 한다."""
    ...
    ch = max(0.62, 0.36 + dh + 0.12)
    _rect(slide, x, y, w, ch, fill=SOFT, radius=True)
    if label:
        _pill(slide, x + 0.14, y + 0.10, label, w=(pw or 1.02), h=0.25,
              size=9.5)
    if right:
        _text(slide, right, x + w - rw - 0.14, y + 0.12, rw, 0.22,
              size=BODY_PT, color=TEAL, bold=True, align=PP_ALIGN.RIGHT,
              fit=False)
    if desc:
        _text(slide, desc, x + 0.16, y + 0.40, dw, max(0.22, dh),
...
_JUNK_RE = re.compile(
    r"^(?:class|Class|CLASS|반)\s*[A-Za-z0-9]?$|^[A-Za-z]\s*반$|^반\s*[A-Za-z]$",
    re.I)


def _is_junk(name):
    """★유미니에 반 이름이 정리되지 않아 'class A' 로 남은 경우가 있다.
    인쇄물에 그대로 나가면 안 되므로 싣지 않는다."""
    t = _s(name)
    if not t:
        return True
    if _JUNK_RE.match(t):
        return True
    return "확인 필요" in t or t in ("-", "—", "미정")
...
def _split_change(val):
    """'30~40점 → 70~80점대' → ('30~40점', '70~80점대').
    화살표가 없으면 ('', 전체)."""
    v = _s(val)
    if not v:
        return "", ""
    parts = [p.strip() for p in _ARROW_RE.split(v) if p.strip()]
    if len(parts) >= 2:
        return parts[0], parts[-1]
    return "", v


def _ach_row_h(label, val, w=CWID):
    """_ach_row 높이."""
    _src, dst = _split_change(val)
    dh = max(0.26, _need_h(dst, w, 13.0, line_spacing=1.18, pad=0.02))
    return 0.30 + dh + (0.22 if _src else 0.0) + 0.14


def _ach_row(slide, x, y, label, val, w=CWID, pw=1.14):
    """실적 한 줄 — 라벨 캡슐을 위에, 도달 숫자를 아래 크게, 출발점을 작게.

    ★값을 캡슐 오른쪽에 두었더니 긴 라벨('자기주도학습 우수지점')에서
      값 폭이 1.3in 로 줄어 줄바꿈이 생기고, 아래 캡션이 그만큼 밀려
      행간이 벙벙해졌다. 세로로 쌓아 폭을 온전히 쓴다.
    """
    label = _s(label)
    src, dst = _split_change(val)
    _hair(slide, x, y, w)
    y += 0.07
    if label:
        _pill(slide, x, y, label, w=min(w - 0.20, max(pw, _txt_w(label, TINY_PT) + 0.24)))
    yy = y + 0.30
    dh = max(0.26, _need_h(dst, w, 13.0, line_spacing=1.18, pad=0.02))
    if dst:
        _text(slide, dst, x, yy, w, dh, size=13.0, color=INK, bold=True,
              line_spacing=1.18, min_size=10.5)
        yy += dh
    if src:
        _text(slide, src + "에서", x, yy, w, 0.22, size=TINY_PT, color=MUTED,
              fit=False)
        yy += 0.22
    return yy + 0.14
...
def _target_rows(schema) -> List[Tuple[str, str, str]]:
    ...
    for it in _items(tg)[:4]:
        ...
        d = _clean_tail(_first(it.get("desc"), it.get("body"),
                                it.get("subj"), default=""))
        # 'class A' 같은 미정리 반 이름은 싣지 않는다
        d = " · ".join(t.strip() for t in re.split(r"[·,]", d)
                       if t.strip() and not _is_junk(t))
        sj = _clean_tail(_s(it.get("subject") or it.get("subjects")))
        if sj == d:
            sj = ""
        # ★값이 비면 라벨만 남은 빈 상자가 된다 → 행 자체를 뺀다
        if g and (d or sj):
            out.append((g, d, sj))
    return out
...
def _norm_cls(text):
    """반 이름 비교용 정규화 — 구분자·공백을 없애고 이어붙인다."""
    return re.sub(r"[^0-9A-Za-z가-힣]", "", _s(text))


def _dup_ratio(names, seen, seen_flat):
    """이 그룹의 반 이름이 수업 대상에 얼마나 겹치는지(0~1).
    ★'고1M / 고1T' 처럼 한 칸에 두 반이 묶여 오는 경우가 있어
      토큰 단위·문자열 단위를 모두 본다."""
    if not names:
        return 0.0
    flat = "".join(seen_flat)
    hit = 0
    for n in names:
        toks = [t for t in re.split(r"[·,/|+\s]+", _s(n)) if t]
        # 이 칸의 반 이름들이 하나라도 수업 대상에 있으면 겹친 것으로 본다
        if any(t in seen for t in toks):
            hit += 1
            continue
        nk = _norm_cls(n)
        if nk and (nk in flat or all(_norm_cls(t) in flat for t in toks if t)):
            hit += 1
...
def _level_rows(schema) -> List[Tuple[str, str]]:
    ...
    # ★'고1M / 고1T' 처럼 슬래시로 묶여 오는 데이터가 있어 토큰이 안 맞았다.
    #   구분자를 넓히고, 토큰 비교가 실패해도 문자열 유사도로 한 번 더 본다.
    seen, seen_flat = set(), []
    for _g, _d, _sj in _target_rows(schema):
        raw_txt = _s(_d) + " " + _s(_sj)
        seen_flat.append(_norm_cls(raw_txt))
        for tok in re.split(r"[·,/|+\s]+", raw_txt):
            tok = tok.strip()
            if tok:
                seen.add(tok)
    ...
    if isinstance(groups, list) and groups:
        for g in groups[:3]:
            ...
            for r in (g.get("rows") or g.get("items") or []):
                ...
                if nm and not _is_junk(nm):
                    names.append(nm)
            body = _clean_tail(" · ".join(names))
            # 수업 대상에 이미 나온 반 이름이 대부분이면 중복이므로 건너뛴다
            if names and (seen or seen_flat):
                if _dup_ratio(names, seen, seen_flat) >= 0.5:
                    continue
            if head and body:
...
def _front_admission(slide, schema, x):
    ...
    # ★인용 박스가 0.92in 을 먹어 단계 카드가 0.65in 로 줄고 설명이 한 줄로
    #   잘렸다. 단계가 많으면 인용을 빼고 단계에 자리를 준다
    #   (같은 문장이 뒷면 네이비 박스에도 있다).
    if len(steps) >= 4:
        quote = ""
    ...
    if fees:
        ...
        y += 0.24
        _fpw2 = _pill_w_uniform([k for k, _v in fees], lo=1.02, hi=1.40)
        for k, v in fees:
            y = _pill_row(slide, x, y, k, v, pw=_fpw2)
...
def _back_why(slide, schema, x):
    ...
    if rows:
        ...
        for k, v in rows:
            y = _ach_row(slide, x, y, k, v, pw=_pw)
...
def _back_features(slide, schema, x):
    ...
    if targets:
        ...
        y += 0.24
        _tpw = _pill_w_uniform([g for g, _d, _sj in targets], size=9.5,
                               lo=0.94, hi=1.40)
        for i, (g, d, sj) in enumerate(targets):
            if y + t_h[i] > Y_BOT:            # 하한 강제 — 겹치기 전에 멈춘다
                break
            y = _soft_row(slide, x, y, g, d, right=sj, pw=_tpw)
