...
def _back_why(slide, schema, x):
    ...
    # ★아래가 1.2in 이상 비면 학원 사진으로 채운다(간격으로 때우지 않는다).
    _as = schema.get("assets") or {}
    _ph = _as.get("photos")
    _pool = [p for p in (_ph if isinstance(_ph, list) else []) if _s(p)]
    if not _pool and _s(_as.get("cover")):
        _pool = [_s(_as.get("cover"))]
    if _pool and (Y_BOT - y) >= 1.20:
        ih = min(2.20, Y_BOT - y - 0.30)
        _label(slide, x, y + 0.06, "학원 전경")
        _photo_or_box(slide, _pool[-1], x, y + 0.34, CWID, ih,
                      {"soft": SOFT, "body": MUTED, "card_line": LINE},
                      "학원 전경", cover_mode=True)
