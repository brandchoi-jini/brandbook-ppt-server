...
def _dup_ratio(names, seen, seen_flat):
    ...
    if not names:
        return 0.0
    hit = sum(1 for n in names if n.strip() in seen)
    if hit:
        return hit / len(names)
    # 토큰이 안 맞으면 이어붙인 문자열 포함 여부로 본다
    flat = " ".join(seen_flat)
    hit = sum(1 for n in names if _norm_cls(n) and _norm_cls(n) in flat)
    return hit / len(names)
    ...
    if isinstance(groups, list) and groups:
        for g in groups[:3]:
            ...
            if names and (seen or seen_flat):
                if _dup_ratio(names, seen, seen_flat) >= 0.6:
                    continue
            if head and body:
